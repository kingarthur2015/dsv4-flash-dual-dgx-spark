...
### v019-ngc2603 (최신, NGC 26.03)

v018에서 vLLM 0.19.1로 업그레이드하여 Gemma 4 지원, 비동기 스케줄링, CUDA 13.2 `cuMemcpyBatchAsync` 호환 패치를 추가. Transformers 5.5.0으로 네이티브 Gemma 4 아키텍처(MoE + Dense, 멀티모달, thinking 모드)를 지원합니다. vLLM V1 엔진 최적화로 TTFT가 v018 대비 **~2배 향상**. 기존 Qwen3.5 양자화 포맷(FP8, NVFP4, INT4) 모두 완벽 지원.
...
### v018-ngc2603 (이전, NGC 26.03)

NGC 26.03 최초 소스 빌드. 네이티브 CUDA 13.2로 26.01의 compat layer 오버헤드를 제거하여 **KV 캐시 +23%** 확보. PyTorch 2.11에서 `_C_stable_libtorch` 컴파일이 가능해져 단일 이미지로 모든 양자화를 지원. Gemma 4 지원과 TTFT 개선이 포함된 v019-ngc2603로 대체됨.
...
| `qwen3.5-122b-fp8.env` | Qwen/Qwen3.5-122B-A10B-FP8 | FP8 (멀티모달) | 2 | v019-ngc2603 |
| `redhatai-122b-nvfp4.env` | RedHatAI/Qwen3.5-122B-A10B-NVFP4 | NVFP4 (사전 양자화) | 1 | v019-ngc2603 |
| `intel-122b-int4.env` | Intel/Qwen3.5-122B-A10B-int4-AutoRound | INT4 AutoRound (Marlin) | 1 | v019-ngc2603 |
| `wangzhang-122b-fp8.env` | wangzhang/Qwen3.5-122B-A10B-abliterated | FP8 (텍스트 전용, 탈검열) | 2 | v019-ngc2603 |
| `wangzhang-122b-nvfp4.env` | wangzhang/Qwen3.5-122B-A10B-abliterated-NVFP4 | NVFP4 (텍스트 전용, 탈검열) | 1 | v019-ngc2603 |
| `qwen3.5-397b-int4.env` | Intel/Qwen3.5-397B-A17B-int4-AutoRound | INT4 AutoRound (Marlin) | 2 | v019-ngc2603 |
| `qwen3.5-122b-nvfp4.env` | Qwen3.5-122B-A10B | NVFP4 (런타임) | 1 | v019-ngc2603 |
| `qwen3.5-122b-nvfp4-tp2.env` | Qwen3.5-122B-A10B | NVFP4 (런타임) | 2 | v019-ngc2603 |
...
```bash
# NGC 26.03 + vLLM 0.19.1 (Gemma 4 + Qwen3.5, 모든 양자화)
...
├── Dockerfile.gemma4           # v019-ngc2603 (NGC 26.03, 최신)
├── Dockerfile.ngc2603-v3       # v018-ngc2603 (NGC 26.03, 이전)
├── models/                     # 검증된 모델 프리셋
...
│   ├── fix_cuda13_memcpy_batch.py # CUDA 13.0+ cuMemcpyBatchAsync 수정
...
| 변수 | 설명 | 예시 |
|---|---|---|
| `VLLM_IMAGE` | 사전 빌드된 Docker 이미지 | `vllm-spark:v019-ngc2603` |
...
| 패치 | 목적 |
|---|---|
| `fix_cuda13_memcpy_batch` | CUDA 13.0+ `cuMemcpyBatchAsync` API 수정 |
